...
export const registerUser = async (data: RegisterInput, req?: Request) => {
  ...
  // Referral side-effects — all best-effort, never block registration.
  void (async () => {
    try {
      // (1) Every user gets their own referral code so they can start
      //     inviting immediately. Idempotent — no-op if the upsert already
      //     ran. We only run after the post-transaction profile exists.
      await ensureReferralCodeForUser(user.id);

      // (2) If the caller supplied a valid ref code, attribute the
      //     invite. Self-referral / IP-cap / program-active guards live
      //     in `claimReferralCode`.
      if (referredByCode) {
        const ip =
...
export const verifyEmail = async (data: VerifyEmailInput) => {
  ...
  // Referral payout: if this user came in via a referral code, mint the
  // REWARDED status, credit both parties, and send in-app notifications.
  // Best-effort — never throws back into the verify response.
  try {
